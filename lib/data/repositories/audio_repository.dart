...
class AudioRepositoryImpl implements AudioRepository {
  ...
  Future<List<AudioModel>> getAudios() async {
    final response = await _httpClient.get(HttpHelper.audiosR);
  ...
  Future<AudioModel?> getAudio(int audioId) async {
    final response = await _httpClient.get("${HttpHelper.audioR}$audioId");
  ...
  Future<void> createAudio(AudioModel audio) async {
    ...
    await _httpClient.post(HttpHelper.audioC, audio.toJson());
  }

  @override
  Future<void> updateAudio(AudioModel audio) async {
    // Send update Audio request to API
    await _httpClient.put(HttpHelper.audioU + audio.id.toString(), audio.toJson());
  }

  @override
  Future<void> deleteAudio(int audioId) async {
    // Send delete Audio request to API
    await _httpClient.delete(HttpHelper.audioD + audioId.toString());
  }
}
